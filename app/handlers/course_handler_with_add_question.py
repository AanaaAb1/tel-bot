...
async def start_exam_selected(update, context):
    """Start exam for selected chapter or show chapters for selected course"""
    query = update.callback_query
    await query.answer()
    ...
    if value.isdigit():
        # It's an exam_id, start the exam
        exam_id = int(value)
        ...
        # It's a course_code, show chapters
        course_code = value
        course_name_map = {
            "bio": "Biology",
            "physics": "Physics",
        ...
        course_name = course_name_map.get(course_code, course_code.title())
