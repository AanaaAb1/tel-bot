...
async def admin_save_question(update, context):
    """Save the completed question to database with proper course/chapter mapping"""
    question_data = context.user_data.get('admin_question_data', {})
    ...
    try:
        ...
        # Use the enhanced question service to add question properly
        from app.services.question_service import add_question_by_course_chapter
        
        # Handle option E if provided
        option_c = question_data.get('option_c', '')
        option_d = question_data.get('option_d', '')
        option_e = question_data.get('option_e', '')
        
        # If option E is provided, store it appropriately
        if option_e and option_e.strip():
            # For now, append to option_c or handle differently
            option_c = f"{option_c} ({option_e})"
        ...
        course_display = course if course != "No Course" else "General"
        chapter_display = chapter if chapter != "No Chapter" else "General"
        
        await safe_edit_message_text(update, 
            f"✅ Question added successfully to {course_display} - {chapter_display}!\n\n"
        ...
        await safe_edit_message_text(update, 
            ...
            f"Please check that:\n"
            f"• Course '{question_data.get('course', 'Unknown')}' exists\n"
            f"• Chapter '{question_data.get('chapter', 'Unknown')}' exists in that course",
            reply_markup=get_admin_questions_menu()
        )
...
# Add question from course (legacy wrapper for backward compatibility)
async def admin_add_question_from_course(update, context):
    """Legacy wrapper for adding questions from course selection"""
    await admin_select_course_for_question(update, context)
